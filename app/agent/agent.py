...
from app.agent.message_processor import MessageProcessor,MessageResponse
from app.dialogue_understanding.flow import FlowsList,FlowLoader
from app.core.stores import TrackerStore,create_tracker_store
from app.policies import EnterpriseSearchPolicy, FlowPolicy, PolicyEnsemble
from app.dialogue_understanding.generator import LLMCommandGenerator
from app.core.tracker import DialogueStateTracker
from app.agent.actions import Action,register_action
from app.dialogue_understanding.processor import CommandProcessor
from app.agent.graph import get_message_processing_graph,create_initial_state
from app.shared.config import LLMConfig
from app.shared.logger import logger
from app.shared import yaml_loader
...
def _load_custom_actions(actions_path:Path) -> list[str]:
    ...
    if not actions_path.exists or not actions_path.is_dir():
        logger.warning(f"[load_custom_actions] actions 目录不存在: {actions_path}")
        return []
    
    registered_actions = []

    # 将actions 目录的父目录添加到sys.path,以便正确导入
    parent_dir = str(actions_path.parent)
    if parent_dir not in sys.path:
        logger.info(f"[load_custom_actions] 将 actions 目录的父目录添加到sys.path: {parent_dir}")
        sys.path.insert(0,parent_dir) # 插入到前面，优先级更高
    
    try:
        # 扫描actions目录下的所有Python文件
        for py_file in actions_path.glob("*.py"):
            ...
            try:
                # 动态导入模块 使用importlib.util.spec_from_file_location 可以避免importlib.import_module的副作用
                spec = importlib.util.spec_from_file_location(module_name,py_file)
                if spec is None or spec.loader is None:
                    logger.warning(f"[load_custom_actions] 无法导入模块: {module_name}，文件不存在或无法加载: {py_file}")
                    continue
                # module_from_spec 创建一个新模块对象
                ...
                # 查找继承自Action基类的类 使用inspect.isclass 检查是否为类 使用inspect.isclass_subclass 检查是否为子类
                for name,obj in inspect.getmembers(module,inspect.isclass):
                    if issubclass(obj,Action) and obj is not Action and obj.__module__ == module_name:
                        try:
                            # 实例化Action类 使用Action的__init__方法
                            action_instance = obj()
                            register_action(action_instance)
                            logger.info(f"[load_custom_actions] 成功注册自定义类: {action_instance.name}")
                            registered_actions.append(action_instance.name)
                        except Exception as e:
                            logger.warning(f"[load_custom_actions] 无法实例化自定义类: {name}，错误: {e}")
            except Exception as e:
                logger.warning(f"[load_custom_actions] 无法导入模块: {module_name}，错误: {e}")
...
class Agent:
    ...
    def __init__(self,
        ...
        if tracker_store :
            ...
            self.tracker_store = create_tracker_store(self.config.tracker_store_type,**self.config.tracker_store_config)
        ...
        self.message_processor = MessageProcessor(
            domain=self.domain,
            flows=self.flows,
            police_ensemble=self.police_ensemble,
    ...
    async def handle_message(
        ...
        initial_state = create_initial_state(
            ...
            policy_ensemble=self.police_ensemble
        )
        # 执行图式
        logger.info(f"[Agent] 使用LangGraph处理消息{message[:50]}...")
        final_state = await self.graph.ainvoke(initial_state)
        logger.info(f"[Agent] 消息处理完成...")

        # 从最终状态提取结果
        update_tracker = final_state.get('tracker',tracker)
        final_response = final_state.get('final_response',[])
        node_history = final_state.get('node_history',[])
        error = final_state.get('error')

        # 保存tracker
        await self.tracker_store.save(update_tracker)
        ...
        logger.info(f"[Agent] 处理完成, 节点路径: {' -> '.join(node_history)},"
                    f"错误: {error}" if error else "无错误"
                    f"响应数: {len(final_response)}"
                    )
        return response
    ...
    def handle_message_sync(
        ...
        import asyncio
        try:
            logger.info(f"[Agent] 同步处理用户消息{message[:50]}...")
            loop = asyncio.get_event_loop()
        except RuntimeError:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
        return loop.run_until_complete(self.handle_message(message,sender_id,metadata))
    ...
    @classmethod
    def load(cls,config_path:str|Path,config:Optional[AgentConfig] = None) -> "Agent":
        """从项目目录或模型压缩包加载Agent。
        ...
        """
        import tarfile
        from app.training.model_storage import extract_model_archive,get_latest_model
