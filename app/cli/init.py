# -*- coding: utf-8 -*-
"""
Customer Server Agent_ai init命令
...
# 默认的config.yml内容
DEFAULT_CONFIG_CONTENT = '''# Customer Server Agent AI 配置文件
# LLM驱动的对话系统配置
...
responses:
  utter_greet:
    - text: "你好！我是Customer Server Agent AI助手，有什么可以帮助你的吗？"
    - text: "您好！很高兴为您服务。"
...
from Customer Server Agent_ai.agent.actions import Action, ActionResult
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        if user_name:
            ...
            result.add_response("你好！我是Customer Server Agent AI助手，有什么可以帮助你的吗？")
        return result
...
# README内容
DEFAULT_README_CONTENT = '''# Customer Server Agent AI 项目
...
1. 安装依赖:
   ```bash
   pip install Customer Server Agent-ai
   ```
...
3. 训练模型:
   ```bash
   Customer Server Agent train
   ```

4. 运行服务:
   ```bash
   Customer Server Agent run
   ```

5. 交互测试:
   ```bash
   Customer Server Agent shell
   ```
...
请参考 Customer Server Agent AI 文档获取更多信息。
'''
...
def init_command(
    ...
    """初始化新的Customer Server Agent AI项目。
    ...
    示例:
        Customer Server Agent init                    # 在当前目录初始化
        Customer Server Agent init --path ./my_bot    # 在指定目录初始化
        Customer Server Agent init --template basic   # 使用basic模板
    """
    click.echo("欢迎使用 Customer Server Agent AI!\n")
    click.echo("正在创建新项目...\n")
    ...
    try:
        ...
        click.echo("  3. 运行 'Customer Server Agent train' 训练模型")
        click.echo("  4. 运行 'Customer Server Agent shell' 测试对话")
        click.echo("")
